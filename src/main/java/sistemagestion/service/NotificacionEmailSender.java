...
public class NotificacionEmailSender {
    ...
    private static String cargarLogoBase64() {
        try (InputStream is = NotificacionEmailSender.class
    ...
    // ── Resolver correo destinatario ──────────────────────────
    private static String resolverCorreo(Notificacion n) {
        if (n.getCorreodestinatario() != null) return n.getCorreodestinatario();
        if (n.getUsuario() != null)            return n.getUsuario().getCorreo();
        return null;
    }
    ...
    private static String[] recomendacionesPorTipo(String tipo) {
        ...
        return switch (tipo.toUpperCase().trim()) {
            ...
            case "SOSPECHOSO", "PERSONA SOSPECHOSA" -> new String[]{
            ...
            case "ANIMAL", "ANIMAL PELIGROSO" -> new String[]{
            ...
            case "EMERGENCIA MÉDICA" -> new String[]{
            ...
            case "DESASTRE NATURAL", "INUNDACIÓN", "DESLIZAMIENTO" -> new String[]{
    ...
    private static String buildHtml(Notificacion n) {
        ...
        if (n.getUsuario() != null) {
            String pn = n.getUsuario().getPrimer_nombre()   != null ? n.getUsuario().getPrimer_nombre()   : "";
            String pa = n.getUsuario().getPrimer_apellido() != null ? n.getUsuario().getPrimer_apellido() : "";
            String nombre = (pn + " " + pa).trim();
            if (!nombre.isEmpty()) nombreUsuario = nombre;
        }

        // Datos de la alerta
        Alerta a = n.getAlerta();

        String tipoAlerta  = (a != null && a.getTipoalerta()  != null) ? a.getTipoalerta().getNombre() : "—";
        String barrio      = (a != null && a.getBarrio()      != null) ? a.getBarrio().getNombre()      : "—";
        ...
        if (a != null && a.getUsuario() != null) {
            String rn = a.getUsuario().getPrimer_nombre()   != null ? a.getUsuario().getPrimer_nombre()   : "";
            String ra = a.getUsuario().getPrimer_apellido() != null ? a.getUsuario().getPrimer_apellido() : "";
            String nombreR = (rn + " " + ra).trim();
            if (!nombreR.isEmpty()) reportadoPor = nombreR;
        }

        // Colores según estado
        String estadoColor = (a != null && a.getEstado() != null) ? switch (a.getEstado()) {
            case PENDIENTE                    -> "#e53935";
            case EN_ATENCION, UNIDAD_ASIGNADA -> "#fb8c00";
            case RESUELTA                     -> "#43a047";
            default                           -> "#6b7280";
        } : "#6b7280";

        String estadoBg = (a != null && a.getEstado() != null) ? switch (a.getEstado()) {
            case PENDIENTE                    -> "#fff0f0";
            case EN_ATENCION, UNIDAD_ASIGNADA -> "#fff8e1";
        ...
        return """
            ...
            <div style="max-width:600px;margin:24px auto;background:#f4f6fb;">
              ...
              <div style="background:#1f3a56;padding:24px 32px;
                ...
                <div>
                  <div style="font-size:20px;font-weight:bold;color:white;
                              letter-spacing:0.3px;">WolertApp</div>
              ...
              <!-- BANNER -->
              <div style="background:#fff0f0;border-bottom:1.5px solid #fecaca;
                          padding:13px 32px;display:flex;align-items:center;gap:10px;">
                <span style="font-size:15px;color:#a32d2d;">⚠</span>
                <span style="font-size:13px;font-weight:bold;color:#a32d2d;">
                  Nueva alerta registrada en tu barrio
                </span>
              </div>
              ...
              <div style="background:white;padding:28px 32px;">
                ...
                <p style="font-size:13px;color:#6b7280;margin:0 0 22px;line-height:1.6;">
                  Se ha registrado una nueva alerta en tu zona.
                  Te informamos para que tomes las medidas necesarias.
                </p>
                ...
                <div style="border:1px solid #e5e7eb;border-radius:12px;
                  ...
                  <div style="background:#f8fafc;padding:11px 16px;
                              border-bottom:1px solid #e5e7eb;
                              display:flex;align-items:center;
                              justify-content:space-between;">
                    <div style="display:flex;align-items:center;gap:8px;">
                      <div style="width:9px;height:9px;border-radius:50%;
                                  background:%s;flex-shrink:0;"></div>
                      <span style="font-size:13px;font-weight:bold;
                                   color:#111827;">%s</span>
                    </div>
                    <span style="font-size:11px;font-weight:bold;
                                 color:%s;background:%s;
                                 padding:3px 12px;border-radius:20px;">%s</span>
                  </div>

                  <table style="width:100%%;border-collapse:collapse;">
                    <tr style="border-bottom:1px solid #f3f4f6;">
                      <td style="padding:10px 16px;font-size:12px;font-weight:bold;
                                 color:#6b7280;width:38%%;">Barrio:</td>
                      <td style="padding:10px 16px;font-size:12px;
                                 color:#111827;">%s</td>
                    </tr>
                    <tr style="border-bottom:1px solid #f3f4f6;">
                      <td style="padding:10px 16px;font-size:12px;font-weight:bold;
                                 color:#6b7280;">Fecha y hora:</td>
                      <td style="padding:10px 16px;font-size:12px;
                                 color:#111827;">%s</td>
                    </tr>
                    <tr style="border-bottom:1px solid #f3f4f6;">
                      <td style="padding:10px 16px;font-size:12px;font-weight:bold;
                                 color:#6b7280;">Reportado por:</td>
                      <td style="padding:10px 16px;font-size:12px;
                                 color:#111827;">%s</td>
                    </tr>
                    <tr>
                      <td style="padding:10px 16px;font-size:12px;font-weight:bold;
                                 color:#6b7280;vertical-align:top;">Descripción:</td>
                      <td style="padding:10px 16px;font-size:12px;
                                 color:#111827;line-height:1.5;">%s</td>
                    </tr>
                  </table>
                </div>
                ...
                <div style="background:#f8fafc;border:1px solid #e5e7eb;
                            ...
                            margin-bottom:8px;">
                  <p style="font-size:13px;font-weight:bold;color:#111827;
                             margin:0 0 10px;">
                    ℹ&nbsp; Recomendaciones para: %s
                  </p>
              ...
              <div style="background:white;border-top:1px solid #e5e7eb;
                ...
                <p style="font-size:12px;color:#6b7280;margin:0 0 4px;">
                  Recibiste este correo porque estás suscrito a alertas de tu barrio.
                </p>
            ...
            """.formatted(
                logoTag(),      // logo en header
                nombreUsuario,  // Hola, {nombre}
                mensaje,        // mensaje notificación
                estadoColor,    // punto color
                tipoAlerta,     // tipo alerta
                estadoColor,    // badge color texto
                estadoBg,       // badge color fondo
                estado,         // PENDIENTE / RESUELTA...
                barrio,         // barrio
                fechaHora,      // fecha y hora
                reportadoPor,   // reportado por
                descripcion,    // descripción
                tipoAlerta,     // título recomendaciones
                listaRecs       // lista dinámica
        );
    }
}