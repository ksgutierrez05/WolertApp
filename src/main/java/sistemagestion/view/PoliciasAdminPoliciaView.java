...
/**
 * Vista: listado de policías.
 * Uso: root.setCenter(new PoliciasAdminPoliciaView(policiaService).build());
 */
public class PoliciasAdminPoliciaView {
    ...
    public ScrollPane build() {
        ...
        Label title = new Label("👮 Policías");
        title.setFont(Font.font("System", FontWeight.BOLD, 22));
        title.setTextFill(Color.web("#111827"));
        content.getChildren().add(title);

        VBox panel = createPanel("Listado de policías");
        try {
            List<Policia> lista = policiaService.listar();
            if (lista.isEmpty()) {
                panel.getChildren().add(label("No hay policías registrados.", 13, GRAY_TEXT, false));
            } else {
                for (Policia p : lista) {
                    ...
                    String rango = p.getRango() != null ? p.getRango() : "—";
                    ...
                    panel.getChildren().addAll(
                            listItem("👮", nombre + " — " + rango, estadoStr, color),
                            separator());
                }
            }
        } catch (Exception e) {
