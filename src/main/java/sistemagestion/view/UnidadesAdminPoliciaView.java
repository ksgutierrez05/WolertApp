...
/**
 * Vista: listado de unidades policiales.
 * Uso: root.setCenter(new UnidadesAdminPoliciaView(unidadService).build());
 */
public class UnidadesAdminPoliciaView {
    ...
    public ScrollPane build() {
        ...
        Label title = new Label("🚓 Unidades policiales");
        title.setFont(Font.font("System", FontWeight.BOLD, 22));
        title.setTextFill(Color.web("#111827"));
        content.getChildren().add(title);

        VBox panel = createPanel("Listado de unidades");
        try {
            List<UnidadPolicial> lista = unidadService.listar();
            if (lista.isEmpty()) {
                panel.getChildren().add(label("No hay unidades registradas.", 13, GRAY_TEXT, false));
            } else {
                for (UnidadPolicial u : lista) {
                    String estadoStr = u.getEstado() != null ? u.getEstado().name().replace("_", " ") : "—";
                    String barrio    = u.getBarrio() != null ? u.getBarrio().getNombre() : "—";
                    String color = u.getEstado() == EstadoUnidadPolicial.OPERATIVA ? GREEN
                                 : u.getEstado() == EstadoUnidadPolicial.ACTIVA    ? ORANGE
                                 : GRAY_TEXT;
                    panel.getChildren().addAll(
                            listItem("🚓", u.getNombre() + " — " + barrio, estadoStr, color),
                            separator());
                }
            }
        } catch (Exception e) {
