...
public class MisAtencionesPoliciaView {
    ...
    private HBox buildTopBar() {
        ...
        Label title = new Label("Historial de Atenciones de la Unidad");
    ...
    private HBox buildStatsRow(List<AtencionAlerta> lista) {
        ...
        long mias = lista.stream().filter(a -> {
            String regPor = extraerRegistradoPor(a).toLowerCase();
            String miUser = usuarioActual != null
                    ? usuarioActual.getUsername().toLowerCase() : "";
    ...
    private VBox buildFiltros(List<AtencionAlerta> todas, VBox timelineContainer) {
        ...
        filtroTxt.setTextFill(Color.web(WHITE));
        ...
        busqueda.focusedProperty().addListener((o, ov, f)
                -> busqueda.setStyle(f ? bBase.replace(BORDER, C_DARK_GRAD) : bBase));
        ...
        List<Pill> pills = List.of(
                new Pill("Todos", null, "\uf0c9", C_DARK_GRAD, WHITE),
                new Pill("Mis atenciones", "MIO", "\uf007", BLUE, "#e3f2fd"), // ← nueva
    ...
    private void renderTimeline(VBox container, List<AtencionAlerta> todas,
        ...
        List<AtencionAlerta> filtradas = todas.stream().filter(at -> {
            // ── Filtro por estado o "MIO" ─────────────────────────
            if (estadoFiltro != null) {
                if ("MIO".equals(estadoFiltro)) {
                    // Solo las registradas por este usuario
                    if (!esMia(at)) {
                        return false;
                    }
                } else {
                    ...
                    if (!at.getEstado().name().equals(estadoFiltro)) {
                        return false;
                    }
                }
            }
            ...
            if (!texto.isEmpty()) {
                ...
                boolean m = false;
                if (at.getDescripcion() != null && at.getDescripcion().toLowerCase().contains(q)) {
                    m = true;
                }
                if (!m && at.getObservacion() != null && at.getObservacion().toLowerCase().contains(q)) {
                    m = true;
                }
                if (!m && at.getUnidad() != null && at.getUnidad().getNombre() != null
                        && at.getUnidad().getNombre().toLowerCase().contains(q)) {
                    m = true;
                }
                if (!m && at.getTipoarma() != null && at.getTipoarma().getNombre() != null
                        && at.getTipoarma().getNombre().toLowerCase().contains(q)) {
                    m = true;
                }
                if (!m && at.getMediotransporte() != null && at.getMediotransporte().getNombre() != null
                        && at.getMediotransporte().getNombre().toLowerCase().contains(q)) {
                    m = true;
                }
    ...
    private HBox buildTimelineItem(AtencionAlerta at, int index, int total) {
        ...
        HBox unidadChip = miniChip("\uf505", unidadNom, "#f0fdf4", GREEN);
        ...
        cardHeader.getChildren().addAll(headerInfo, rightSide);
